				...
				\resizebox{\textwidth}{!}{%
					\begin{tikzpicture}
						\begin{axis}[
							...
							% --- Histogram data ---
							\addplot+ [fill=black,draw=black,ybar interval,mark=no] plot coordinates {
								(-15, 48)
								(-13, 52)
								(-11, 46)
								(-9, 50)
...
%							]
%							
%							\addplot[only marks, mark=|, mark size=5pt, blue, opacity=0.6] table {Images/timestamps.txt};
%						\end{axis}
%					\end{tikzpicture}
%				}
				\centering
				\includegraphics[width=\textwidth]{Images/ExampleCountsExp.pdf}
				\end{minipage}
			}
			\vfill
			\visible<3->{
...
%							xlabel={Value},
%							ylabel={Probability},
%							title={Histogram from Data File},
%							grid=both,
...
%							]
%							
%							% Import data file: first column = x, second = y
%							\addplot+[ybar, fill=blue!40!white, draw=black]
%							table [x index=0, y index=1, col sep=tab] {Images/histogram.txt};
%							
%						\end{axis}
%					\end{tikzpicture}
%				}
				\centering
				\includegraphics[width=\textwidth]{Images/ExampleHistoExp.pdf}
				\end{minipage}
			}
		\end{minipage}
	...
	\begin{frame}{Simulation}
		\begin{minipage}{.45\textwidth}
			\begin{minipage}{\textwidth}
				\centering
				{\tiny \hspace*{2em} $\eta_{\text{idl}}$ = 70\%, $\eta_{\text{sig}}$ = 70\%, 		$R_{\text{noise,idl}}$ = 0$~\text{Hz}$ }
				\includegraphics[width=.75\textwidth]{Images/SweepSinglesIdler_Eff_70_70_NoNoise.pdf}
			\end{minipage}
			%\vfill
			\\ [1.5em]
			\begin{minipage}{\textwidth}
				\centering
				{\tiny \hspace*{1.7em} $\eta_{\text{idl}}$ = \textcolor{red}{0.09\%}, $\eta_{\text{sig}}$ = \textcolor{red}{2.6\%}, 		$R_{\text{noise,idl}}$ = 0$~\text{Hz}$ }
				\includegraphics[width=.75\textwidth]{Images/SweepSinglesIdler_Eff_2-6_009_NoNoise.pdf}
			\end{minipage}
		\end{minipage}
		\hfill
		\begin{minipage}{.45\textwidth}
			\centering
			{\scriptsize \hspace*{2.2em} $\eta_{\text{idl}}$ = 0.09\%, $\eta_{\text{sig}}$ = 2.6\%, 		$R_{\text{noise,idl}}$ = \textcolor{red}{20$~\text{kHz}$} }
	...
	\begin{frame}{Simulation}
		...
		\begin{minipage}{0.3\textwidth}
			\centering
			\includegraphics[width=\textwidth]{Images/SweepSinglesIdler_Eff_90_90_NoNoise.pdf}
			%\vspace{2mm}
			\footnotesize
			\begin{align*}
				\eta_{\text{idl}} &= 90\% \\
				\eta_{\text{sig}} &= 90\% \\
				R_{\text{noise,idl}} &= 0~\text{Hz}
			\end{align*}
		\end{minipage}
		}
		\hfill
		...
		\begin{minipage}{.3\textwidth}
			...
			\footnotesize
			\begin{align*}
				\eta_{\text{idl}} &= \textcolor{red}{0.09\%} \\
				\eta_{\text{sig}} &= \textcolor{red}{2.6\%} \\
				R_{\text{noise,idl}} &= 0~\text{Hz}
			\end{align*}
		\end{minipage}
		}
		...
		\begin{minipage}{.3\textwidth}
			...
			\begin{align*}
				\eta_{\text{idl}} &= 0.09\% \\
				\eta_{\text{sig}} &= 2.6\% \\
				R_{\text{noise,idl}} &= \textcolor{red}{20~\text{kHz}}
			\end{align*}
		\end{minipage}
		}
	\end{frame}
