...
\begin{document}
	...
	\begin{frame}{Motivation}
			...
			\begin{tikzpicture}[baseline, node distance=0.5cm and 5cm]
				% Top image
				\node (a) {\includegraphics[width=.25\textwidth]{Images/NoisyImage.pdf}};
				% Bottom image
				\node (b) [below=of a] {\includegraphics[width=.25\textwidth]{Images/BetterImage.pdf}};
				% Left image, vertically centered between a and b
				\node (p) [left=of $(a)!0.5!(b)$] {\includegraphics[width=.25\textwidth]{Images/PureImage.pdf}};
				
				% Arrows
				\draw[->, thick] (p.east) -- node[above] {add noise} (a.west);
				\draw[->, thick, bend left=20] (a.east) to node[right] {$|\phi\rangle$ ?} (b.east);
			\end{tikzpicture}
	\end{frame}
	...
	\begin{frame}{Photon statistics}
		...
		\begin{minipage}{.5\textwidth}
			...
			\begin{aligned}
				...
				\operatorname{Var}(n) &= \langle n\rangle (Poisson)
			\end{aligned}
			\]
			}
			\visible<3->{
			...
			\begin{aligned}
				...
				\operatorname{Var}(n) &= \langle n\rangle \left(1+\frac{\langle n\rangle}{m}\right) (mmBE)
			\end{aligned}
			\]
			}
		\end{minipage}
