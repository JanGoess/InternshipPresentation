	...
	\begin{center}
		...
		\begin{tikzpicture}[baseline, node distance=1cm and 2cm]
			...
			\visible<3->{
				...
				\draw[-{Latex[length=2mm]}, thick] (p.west) to[bend right=20] node[right] {correlations \footnote<4->{\authortitlefootcite{bridaExperimentalRealizationSubshotnoise2010}}} (a.west);
			...
			\visible<5->{
				\node (c) [right=of a] {\includegraphics[width=.35\textwidth]{Images/CoincImageNoise}};
				% Arrows
				
				\draw[-{Latex[length=2mm]}, thick] (a.east) to node[above] {??} (c.west);
			}
		\end{tikzpicture}
	\end{center}
	\vspace*{2em}
