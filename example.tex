...
\begin{document}
	...
	\begin{frame}{Motivation}
			...
			\begin{tikzpicture}[baseline, node distance=0.5cm and 5cm]
				% Top image
				\node (a) {\includegraphics[width=.1\textwidth]{Images/NoisyImage.pdf}};
				% Bottom image
				\node (b) [below=of a] {\includegraphics[width=.1\textwidth]{Images/BetterImage.pdf}};
				% Left image, vertically centered between a and b
				\node (p) [left=of $(a)!0.5!(b)$] {\includegraphics[width=.1\textwidth]{Images/PureImage.pdf}};
				
				% Arrows
				\draw[-{Latex[length=2mm]}, thick] (p.east) -- node[above,rotate=28] {add noise} (a.west);
				\draw[-{Latex[length=2mm]}, thick, bend left=20] (a.east) to node[right] {$|\phi\rangle$ \footnote{\authortitlefootcite{bridaExperimentalRealizationSubshotnoise2010}}} (b.east);
			\end{tikzpicture}
			\vspace*{2em}
			\textbf{Objective:} Can coincidence measurements provide advantages in terms of precision in noisy regimes?
	\end{frame}
